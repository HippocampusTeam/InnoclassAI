...
class NeuralNetwork:
    def __init__(self, size):  # Создааем объект класса NeuralNetwork, передавая ему список размеров каждого слоя
        ...
        # weights[layer][previous_neuron][current_neuron] - вес между нейроном под номером current в слое layer и
        # нейроном ПРЕДЫДУЩЕГО слоя под номером prev
        ...
        for layer in range(1, len(self.neurons)):  # Проходимся по всем остальным слоям
            ''' TODO: Добавь пустой список для текущего слоя в weights '''
            for previous_neuron in range(len(self.neurons[layer - 1])):  # Проходимся по всем нейронам ПРЕДЫДУЩЕГО слоя
                ...
                for current_neuron in range(len(self.neurons[layer])):  # Проходимся по всем нейронам ТЕКУЩЕГО слоя
                    ...
                    # Добавляем вес между нейроном под номером current в слое layer и
                    # нейроном ПРЕДЫДУЩЕГО слоя под номером prev. Изначально равен 0
    ...
    def calculate(self, input):
        self.neurons[0] = input  # Устанавливаем значения нулевого слоя (входные значения сети)
        for layer in range(1, len(self.neurons)):  # Проходимся по всем слоям, кроме нулевого
            for current_neuron in range(len(self.neurons[layer])):  # Проходимся по всем нейронам ТЕКУЩЕГО слоя
                self.neurons[layer][current_neuron] = 0 # Обнуляем значение до начала подсчёта
                for previous_neuron in range(len(self.neurons[layer - 1])):  # Проходимся по всем нейронам ПРЕДЫДУЩЕГО слоя
                    self.neurons[layer][current_neuron] += '''TODO: Рассчитайте значение текщего нейрона, исходя из нейронов предыдущего слоя'''
                    
                neuron_value = '''TODO: Вычислите сумму значения с учётом bias текущего нейрона'''
